...
public class GestorRevisionPagos implements Serializable 
{
	...
	public void buscarActivos (ActionEvent actionEvent)
	{
		if (ConnectionManager.comprobarConexion())
		{
			...
			if (sNURCATB.isEmpty())
			{
				...
				this.setTablaactivos(CLPagos.buscarActivosConPagos(filtro));
	...
	public void seleccionarPagoActivo(ActionEvent actionEvent) 
    { 
		if (ConnectionManager.comprobarConexion())
		{
	    	if (gastoactivoseleccionado!=null)
	    	{
		    	...
		    	cargarPago(gastoactivoseleccionado);
	    	}
	    	else
	    	{
		    	FacesMessage msg;
	...
	public void seleccionarPagoProvision(ActionEvent actionEvent) 
    { 
		if (ConnectionManager.comprobarConexion())
		{
	    	if (gastoprovisionseleccionado!=null)
	    	{
		    	...
		    	cargarPago(gastoprovisionseleccionado);
	    	}
	    	else
	    	{
		    	FacesMessage msg;
	...
	public void cargarPago(GastoTabla gastoseleccionado) 
    { 
		if (ConnectionManager.comprobarConexion())
		{
	    	FacesMessage msg;
	    	...
	    	if (gastoseleccionado != null)
	    	{
		    	...
		    	this.sCodGastoB = Long.toString(liCodGasto);
				...
				this.bAbono = gasto.getCOSIGA().equals(ValoresDefecto.DEF_GASTO_ABONADO);
				...
				this.liCodPago = CLPagos.buscarCodigoPago(liCodGasto);
				
				Pago pago = CLPagos.buscarPagoGasto(liCodGasto);
				...
				this.bRecargo = pago.getsRecargoAdicional().equals("0");
				
				if (bRecargo)
				{
					this.sValorRecargo = pago.getsRecargoAdicional();
				...
				else
				{
					...
					this.sValorRecargo = Utils.recuperaRecargo(pago.getsRecargoAdicional().startsWith("-"),pago.getsRecargoAdicional());
					
					this.sTipoRecargo = ValoresDefecto.DEF_RECARGO_FIJO;
				...
				this.liValor = gasto.getValor_total();
				this.sRecargo = pago.getsRecargoAdicional();
	    		...
	    		if (sTipoPago.equals(ValoresDefecto.DEF_PAGO_DEVOLUCION))
				{
	    		...
	    		{
	    			logger.debug("TIPO PAGO DESCONOCIDO!!!");
	    			bError = true;
	    		}
